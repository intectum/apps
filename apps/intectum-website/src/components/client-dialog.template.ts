...
const renderClientDialogHTML = (client: Client) => `
  <dialog class="u-panel--invert u-fc u-gap u-p">
    <div class="u-fc u-gap u-center">
      ...
      <div class="u-fr u-gap">
        <a href="/projects?client=${client.slug}" title="Projects">
          ${renderFolderOpenSvg()}
        </a>
        <a href="${client.link.url}" title="${client.link.title ?? 'Website'}">
    ...
    ${client.reference ? `
      <h5>Reference</h5>
      <p>
        ${client.reference}. See <a href="${cvUrl}">my full CV</a> for contact details.
      </p>
    ` : ''}
  </dialog>
`;
