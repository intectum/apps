...
export const get = async (context: Context, id: string) =>
{
  const result = await context.client.query<FullUser & Address & { address_id: string }>(
    'SELECT "user".id, "user".email, "user".status, "user".admin, "user".name, "user".image, "user".gender, "user".terms_accepted, "user".contacts, "user".groups, "user".pending, address.id as address_id, address.latitude, address.longitude, address.meta FROM "user" LEFT JOIN address ON address.user_id = "user".id WHERE "user".id = $1',
    [ id ]
  );
...
{
  let fields = [ 'id', 'name', 'image', 'gender', 'contacts', 'groups' ];
...
export const update = async (context: Context, user: User) =>
{
  ...
  await context.client.query<User>(
    'UPDATE "user" SET gender = $1, terms_accepted = $2, contacts = $3, groups = $4, pending = $5 WHERE id = $6',
    [ user.gender, user.terms_accepted, user.contacts, user.groups, pending, user.id ]
  );
